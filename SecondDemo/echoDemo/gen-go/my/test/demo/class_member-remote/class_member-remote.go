...
func Usage() {
  ...
  fmt.Fprintln(os.Stderr, "  bool IsNameExist(i64 callTime, string name)")
  fmt.Fprintln(os.Stderr)
  os.Exit(0)
}
...
func main() {
  ...
  client := demo.NewClassMemberClient(thrift.NewTStandardClient(iprot, oprot))
  ...
  case "IsNameExist":
    if flag.NArg() - 1 != 2 {
      fmt.Fprintln(os.Stderr, "IsNameExist requires 2 args")
      flag.Usage()
    }
    ...
    value1 := argvalue1
    fmt.Print(client.IsNameExist(context.Background(), value0, value1))
    fmt.Print("\n")
    break
  case "":
    Usage()
